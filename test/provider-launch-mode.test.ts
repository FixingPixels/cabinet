...
import { cursorCliProvider } from "../src/lib/agents/providers/cursor-cli";
...
test("Cursor keeps detached prompt runs in one-shot mode", () => {
  assert.equal(resolveDetachedPromptLaunchMode(cursorCliProvider, "Review this"), "one-shot");
  assert.equal(resolveDetachedPromptLaunchMode(cursorCliProvider), "session");
});
